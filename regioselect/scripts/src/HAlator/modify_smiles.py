...
def remove_mapping_mol_to_smi(mol):
...
def get_identical_atoms(smiles) -> str:
    """To get the rank, index and the atom of identical atoms"""
    if isinstance(smiles, str) and len(smiles) >= 1:
        rd_mol = Chem.MolFromSmiles(smiles)  #'c1(F)c(F)cccc1'
        can_smiles = Chem.MolToSmiles(rd_mol, canonical=True)
        new_mol = Chem.MolFromSmiles(can_smiles)
        # new_mol = Chem.AddHs(new_mol)
        rank = list(Chem.CanonicalRankAtoms(new_mol, breakTies=False))
        # unique ranks are just the unique symmetry class
        unique_ranks = set(rank)
        # print(rank)
        # print(f"unique rank: {unique_ranks}")

        for i, j_rank in enumerate(rank):
            new_mol.GetAtomWithIdx(i).SetIntProp("molAtomMapNumber", j_rank + 1)

        """Create a ranked_dict where the keys are """
        ranked_dict = dict.fromkeys(unique_ranks, 0)
        for key in ranked_dict:
            value = [
                (i, new_mol.GetAtomWithIdx(i).GetSymbol())
                for i, j_rank in enumerate(rank)
                if j_rank == key
            ]
            zipped_value = list(zip(*value))
            # print(zipped_value)
            ranked_dict[key] = zipped_value

    return new_mol, can_smiles, rank, ranked_dict
...
def remove_Hs(
    ...
    for atom_idx, H_indices in dict_atom_idx_to_H_indices.items():
        ...
        rdkit_mol2.GetAtomWithIdx(atom_idx).SetFormalCharge(delta_charge)
...
def remove_Hs_halator(
    ...
    rdkit_mol, can_smiles, rank, ranked_dict = get_identical_atoms(smiles)
    first_elements = [
        values[0][0] for values in ranked_dict.values() if "C" in values[1]
    ]
    ...
    dict_atom_idx_to_H_indices = {
        ...
        and atom.GetIdx() in first_elements
    }  # note if atom.GetSymbol() == "C" is note invoked, O-H, N-H, etc. will also get deprotonated.
    dict_atom_idx_to_H_indices = {
        k: v for k, v in dict_atom_idx_to_H_indices.items() if v
    }  # removes empty lists
    ...
    for atom_idx, H_indices in dict_atom_idx_to_H_indices.items():
        ...
        if remove_H:
            rdkit_mol2 = Chem.RemoveHs(rdkit_mol2)
        smi2_nomap = remove_mapping(Chem.MolToSmiles(rdkit_mol2))
        for a in rdkit_mol2.GetAtoms():
            a.SetAtomMapNum(a.GetIdx() + 1)
        smi2_map = Chem.MolToSmiles(rdkit_mol2)
        # smi2_nomap = remove_mapping_mol_to_smi(rdkit_mol2)
        charge_deprot = Chem.GetFormalCharge(rdkit_mol2)
        if rxn == "rm_hydrogen":
            name_deprot = name_deprot = f"{name}#rad{charge_deprot}={atom_idx+1}"
        else:
            name_deprot = f"{name}#{charge_deprot}={atom_idx+1}"
        lst_idx_deprot_mol.append(
            (atom_idx, atom_idx + 1, smi2_nomap, smi2_map, rdkit_mol2, name_deprot)
        )
...
if __name__ == "__main__":
    ...
    print("######################")
    for test_deprotonate, test_removeHs, test_remove_hydrides in zip(
        lst_deprotonate, lst_removeHs, lst_remove_hydrides
    ):
        print("deprotonate")
        ...
        print(test_removeHs)
        print("######################")
        print("remove_hydrides")
        print(test_remove_hydrides)
        print("\n")
